...
import com.set_job_portal.Repository.JobPostRepository;
...
public class JobPostService 
{
	...
	public JobPostDTO createJob(JobPostDTO dto)
	{
		...
		job.setRecruiterEmailId(dto.getRecruiterEmailId());
	...
	public List<JobPostDTO>findByRecruiterEmail(String recruiterEmail)
	{
		return jobPostRepository.findByRecruiterEmailid(recruiterEmail).stream().map(this::mapToDTO).collect(Collectors.toList());
	...
	private JobPostDTO mapToDTO(JobPost post)
	{
		...
		dto.setRecruiterEmailId(post.getRecruiterEmailId());
